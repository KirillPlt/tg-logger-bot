...
async def test_runtime_settings_service_can_disable_and_enable_reaction_logs() -> None:
    ...
    disabled_settings, disabled_changed = await service.set_reaction_logging_enabled(
        False
    )
...
async def test_runtime_settings_service_can_store_greeting() -> None:
    service = BotRuntimeSettingsService(InMemoryBotRuntimeSettingsRepository())

    updated_settings, was_changed = await service.set_greeting("<b>Привет!</b>")

    assert was_changed is True
    assert updated_settings.greeting_html == "<b>Привет!</b>"
    assert await service.get_greeting() == "<b>Привет!</b>"
