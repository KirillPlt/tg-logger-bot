    ...
    async def get(self, chat_id: int) -> ChatState | None:
        ...
        async with self._database.connect() as connection:
            try:
                ...
                self._database.observe_db_operation("chat_state.get", "error", started_at)
                self._database.logger.exception("chat_state_get_failed", extra={"chat_id": chat_id})
                raise
    ...
    async def upsert(self, state: ChatState) -> None:
        ...
        async with self._database.connect() as connection:
            try:
                ...
                self._database.observe_db_operation("chat_state.upsert", "error", started_at)
