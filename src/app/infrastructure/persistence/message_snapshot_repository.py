...
class SQLiteMessageSnapshotRepository(MessageSnapshotRepository):
    ...
    async def get(self, chat_id: int, message_id: int) -> MessageSnapshot | None:
        ...
        with self._database.trace_operation("message_snapshot.get"):
            async with self._database.connect() as connection:
                try:
                    ...
                    self._database.observe_db_operation(
                        "message_snapshot.get", "error", started_at
                    )
        ...
        if row is None:
            self._database.observe_db_operation(
                "message_snapshot.get", "success", started_at
            )
            return None
        ...
        return MessageSnapshot(
            ...
            rendered_html=str(row["rendered_html"])
            if row["rendered_html"] is not None
            else None,
        )

    async def upsert(self, snapshot: MessageSnapshot) -> None:
        started_at = perf_counter()
        with self._database.trace_operation("message_snapshot.upsert"):
            async with self._database.connect() as connection:
                try:
                    ...
                    self._database.observe_db_operation(
                        "message_snapshot.upsert", "error", started_at
                    )
                    self._database.logger.exception(
        ...
        self._database.observe_db_operation(
            "message_snapshot.upsert", "success", started_at
        )
